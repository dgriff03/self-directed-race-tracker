...
**Starting early:** Garmin polling opens one hour before the scheduled start. Earlier points are ignored. The first accepted GPS fix within that window starts the live display and records an actual start time for elapsed time, pace, and splits; the scheduled time stays unchanged. This uses a GPS timestamp, not movement detection, so a stationary tracker near the start can start the clock. Late starts continue to use the scheduled time. Polling runs every five minutes.

...
- `pollGarmin` runs **every minute**, with Garmin requests deferred until near the predicted delivery, independent of viewers. It queries active jobs, claims transaction leases, and fetches KML since the previous fix. Every active tick writes a heartbeat; attempted Garmin requests also update delivery cadence and success/failure state. It removes expired rate-limit entries in bounded batches.

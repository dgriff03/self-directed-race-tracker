...
Aid crossings interpolate between confirmed positions. Moving pace uses a rolling window of recent fixes (~15–20 minutes), falling back to overall average pace when fixes are sparse. While a runner is stopped at an aid station, status reports "At aid station", pace reflects resting state, and estimated map location is pinned at the station. ETAs to downstream stops assume a 10-minute stop at each intermediate aid station, and when currently at an aid station, ETAs use overall average pace plus the remaining dwell time at the current station. Overdue ETAs keep the projected date/time and show minutes overdue. Dates accompany times for multi-day races. Estimated position is clearly marked and projects rolling pace for at most ten minutes.

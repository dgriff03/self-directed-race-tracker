...
await p.waitForTimeout(800);await p.context().setOffline(true);await p.reload();await p.getByRole('heading',{name:body.name,exact:true}).waitFor();await p.context().setOffline(false);
await p.goto('http://127.0.0.1:4173/replay');await p.getByPlaceholder('Race slug, UUID or viewer URL').fill(slug);await p.getByRole('button',{name:'Load event',exact:true}).click();await p.getByText('This event has no recorded locations yet.',{exact:true}).waitFor();
...
}finally{fs.writeFileSync(configPath,original);await b.close();if(created?.data?.id){const id=created.data.id;await fetch(db(''),{method:'PATCH',headers:admin,body:JSON.stringify({[`races/${id}`]:null,[`courses/${id}`]:null,[`jobs/${id}`]:null,[`slugs/${slug}`]:null,[`slugs/${slug}-new`]:null})});}}

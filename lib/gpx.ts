import { cumulative, type Coordinate } from "../shared/race";
export function parseGpxWithElevation(text: string): {
  ...
  const valid = elevations.map((e, i) => Number.isFinite(e) ? i : -1).filter(i => i >= 0);
  if (valid.length >= 2) {
    for (let i = 0; i < valid[0]; i++) elevations[i] = elevations[valid[0]];
    for (let k = 1; k < valid.length; k++) {
      const a = valid[k - 1], b = valid[k];
      for (let i = a + 1; i < b; i++) {
        const f = ds[b] > ds[a] ? (ds[i] - ds[a]) / (ds[b] - ds[a]) : (i - a) / (b - a);
        elevations[i] = elevations[a] + f * (elevations[b] - elevations[a]);
      }
    }
    for (let i = valid.at(-1)! + 1; i < elevations.length; i++) elevations[i] = elevations[valid.at(-1)!];
  }
  const profile = valid.length >= 2 ? elevations : null;
  const indices = simplifyRoute(route, profile);
  return { route: indices.map(i => route[i]), elevationsM: profile ? indices.map(i => profile[i]) : null };
}
...
function simplifyRoute(route: Coordinate[], elevations: number[] | null): number[] {
  if (route.length <= 6000) return route.map((_, i) => i);
  const scale = Math.cos(route[0][1] * Math.PI / 180);
  const points = route.map((p, i) => [p[0] * 111320 * scale, p[1] * 111320, elevations?.[i] ?? 0]);
  const simplify = (tolerance: number) => {
    ...
    while (stack.length) {
      ...
      let best = tolerance*tolerance, index = -1;
      for (let i = a + 1; i < b; i++) {
        const f = Math.max(0, Math.min(1, points[i].reduce((s, v, j) => s + (v-points[a][j])*delta[j], 0)/(length || 1)));
        const error = points[i].reduce((s, v, j) => s + (v-points[a][j]-f*delta[j])**2, 0);
  ...
  let tolerance = 0.5, indices = simplify(tolerance);
  while (indices.length > 6000) { tolerance *= 2; indices = simplify(tolerance); }
  return indices;
}
